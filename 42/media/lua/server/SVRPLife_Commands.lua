...
SVRPLife.Commands = SVRPLife.Commands or {}
...
local function cmdCheck(player, args)
    ...
    if not targetName then
        reply(player, "[SVRPLife] Usage: /lifes check <PlayerName>")
        return
    end

    local username = Admin.resolveTargetUsername(targetName)
    if not username then
        reply(player, "[SVRPLife] Player not found: " .. targetName)
        return
    end

    local ok, msg = Admin.executeOperation(player, SVRPLife.ACTION_VIEW, username)
    reply(player, msg)
end
...
local function cmdAdd(player, args)
    ...
    if not targetName or not amount then
        reply(player, "[SVRPLife] Usage: /lifes add <PlayerName> <amount>")
        return
    end
    if amount <= 0 then
        reply(player, "[SVRPLife] Amount must be positive.")
        return
    end
    ...
    local ok, msg = Admin.executeOperation(player, SVRPLife.ACTION_ADD, username, amount)
    reply(player, msg)
end
...
local function cmdRemove(player, args)
    ...
    local amount     = tonumber(args[2])
    if not targetName or not amount then
        reply(player, "[SVRPLife] Usage: /lifes remove <PlayerName> <amount>")
        return
    end
    if amount <= 0 then
        reply(player, "[SVRPLife] Amount must be positive.")
    ...
    local ok, msg = Admin.executeOperation(player, SVRPLife.ACTION_REMOVE, username, amount)
    reply(player, msg)
end
...
local function cmdSet(player, args)
    ...
    local amount     = tonumber(args[2])
    if not targetName or not amount then
        reply(player, "[SVRPLife] Usage: /lifes set <PlayerName> <amount>")
        return
    end
    ...
    local ok, msg = Admin.executeOperation(player, SVRPLife.ACTION_SET, username, amount)
    reply(player, msg)
end
...
local function cmdDebug(player, _args)
    ...
    local count   = 0
    local parts   = { "[SVRPLife] DEBUG REPORT" }
    ...
    parts[#parts+1] = string.format("  SVRPLife version: %s", SVRPLife.VERSION)
    parts[#parts+1] = string.format("  EnableSystem: %s",
        tostring(SVRPLife.getSandboxCfg("EnableSystem", SVRPLife.DEFAULT_ENABLE_SYSTEM)))
    parts[#parts+1] = string.format("  StartingLives: %d",
        SVRPLife.getSandboxCfg("StartingLives", SVRPLife.DEFAULT_STARTING_LIVES))

    -- Active death cooldowns
    local cooldowns = SVRPLife.DeathHandler and SVRPLife.DeathHandler.getCooldownTable() or {}
    local cdCount   = 0
...
-- Called from OnServerCommand in SVRPLife_Server.lua
-- ============================================================
function Cmds.handleChatCommand(player, fullCommand)
    ...
    if not SVRPLife.isAuthorised(player) then
        reply(player, "[SVRPLife] Access denied.")
        return true
    end
    ...
    if not handler then
        reply(player, "[SVRPLife] Unknown sub-command: " .. sub .. "\n" .. USAGE)
        return true
    end
